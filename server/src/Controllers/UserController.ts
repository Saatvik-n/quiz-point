...
export async function loginUser(req: Request, res: Response, next:NextFunction) {
  ...
  const accessToken = JWT.sign(user._id, process.env.JWT_SECRET_KEY!)
  ...
  return res.status(200).cookie('JWT', accessToken, {
    ...
    if (error.isJoi) {
      return next(HttpError(401, "Make sure that all fields have 5 characters"))
    }
...
export async function getUserQuizzes(
  ...
  try {
    ...
    return next(HttpError(500, "Internal server error while fetching list of user quizzes"))
  }
}
...
export async function validateNext(req:Request, res: Response, next: NextFunction) {
  return res.status(200).json({
