...
const checkSingleOptionCorrect = (userAnsers: boolean[], correctAnswers: singleOption[]):boolean => {
...
const checkMultipleChoiceCorrect = (userAnsers: boolean[], correctAnswers: singleOption[]):boolean => {
  for (let i = 0; i < correctAnswers.length; i++) {
...
const calculateCorrectAnsers = (quizData: QuizData, currentAns: AnswerArray, correctFlashAns: boolean[]) => {
  ...
  for (let i = 0; i < quizData.length; i++) {
    ...
    if (currentQuestion.type === "Single Option") {
      if (checkSingleOptionCorrect(currentAns[i] as boolean[],quizData[i].answerOptions! )) {
        singleOptionCorrect++;
      }
    }
  ...
  return {
    singleOptionCorrect, multipleChoiceCorrect, flashcardCorrect
  }
}
...
const QuizResults: React.FC<QuizResultsProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const correctAnsResult = calculateCorrectAnsers(quizData, currentAns, correctFlashAns)
    ...
    <>
      ...
      <VStack spacing="30px"
       ...
       >
        ...
        <Button size="lg" 
        onClick={() => setSeeDetailedResults(!seeDetailedResults)}> See question and answers </Button>
        {
          seeDetailedResults ? (
            <QuizQATable
            quizData={quizData}
