...
const SingleOptionType: React.FC<SingleOptionTypeProps> = (props) => {
  const { optionText, handleTextChange, index, isCorrect, changeOption, deleteOption} = props;

  return (
    <>
      <HStack marginBottom="10px" width="95%">
        {/* This is wrapped in a box, because by default, Radio is a span */}
        <Radio display="inline-block" isChecked={isCorrect} onClick={() => changeOption(index)} ></Radio>
        <Input
          type="text"
        ...
        />
        <DeleteIcon  display={index > 1 ? "inherit" : "none"}
        cursor="pointer"
